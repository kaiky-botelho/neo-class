...
const LancarNotas: React.FC = () => {
  ...
  useEffect(() => {
    ...
    todasNotas.forEach(n => {
      if (n.turmaId === turma.id && n.alunoId != null && n.bimestre != null) {
        const bi = n.bimestre -1;
        if (init[n.alunoId] && bi>=0 && bi<4) init[n.alunoId][bi] = n.valor?.toString() ?? "";
      }
    });
    setNotas(init);
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    const dtos: NotaDTO[] = alunosDaTurma.map(a=>{
      ...
      const existente = todasNotas.find(n=>
        n.alunoId===a.id && n.turmaId===turma.id && n.bimestre===bimestre
      );
      return { id: existente?.id, alunoId:a.id!, turmaId:turma.id!, materiaId:materiaObj.id!, bimestre, valor:valorNum };
    }).filter(d=>d.valor>0);
